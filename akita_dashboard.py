...
image_path = "akita_action_card.jpg"  # Update with relative image path
...
if image:
    ...
    if change_24hr is not None:
        ...
        change_text = f"24hr Change: {change_24hr:.2f}%"
        change_bbox = draw.textbbox((0, 0), change_text, font=font_medium)
        change_width = change_bbox[2] - change_bbox[0]
        y_change_top = y_price_top + price_width + field_spacing
        draw.text((x_right_change - change_width // 2, y_change_top), change_text, fill=change_color, font=font_medium)
